...
from utils.tools import get_tools, get_tools_by_name
from utils.utils import parse_email, format_email_markdown
    ...
    return {
        "messages": [
            llm_with_tools.invoke([
                {"role": "system", "content": ACTION_INSTRUCTIONS.format(today=datetime.now().strftime("%Y-%m-%d"))}
            ] + state["messages"])
        ]
    }
...
def triage_router(state: State) -> Command[Literal["response_agent", "__end__"]]:
    ...
    result = llm_router.invoke([
        ...
        {"role": "user", "content": f"Please determine how to handle the below email thread:\n\nFrom: {author}\nTo: {to}\nSubject: {subject}\n{email_thread}"},
    ])
