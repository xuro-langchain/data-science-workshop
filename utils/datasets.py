...
response_criteria_1 = "• Send email with write_email tool call to acknowledge the question and confirm it will be investigated"
response_criteria_2 = "• No response needed\n• Ensure this is ignored"
response_criteria_3 = "• No response needed\n• Ensure the user is notified"
response_criteria_4 = "• Check calendar availability for Tuesday or Thursday afternoon next week with check_calendar_availability tool call\n• Confirm availability for a 45-minute meeting\n• Send calendar invite with schedule_meeting tool call\n• Send email with write_email tool call to acknowledge tax planning request and notifying that a meeting has been scheduled"
response_criteria_5 = "• No response needed\n• Ensure the user is notified"
response_criteria_6 = "• Express interest in attending TechConf 2025\n• Ask specific questions about AI/ML workshops\n• Inquire about group discount details\n• Send email with write_email tool call"
response_criteria_7 = "• Explicitly agree to review the technical specifications\n• Acknowledge Friday deadline\n• Send email with write_email tool call"
response_criteria_8 = "• Send email with write_email tool call to express interest in registering daughter for swimming class"
response_criteria_9 = "• No response needed\n• Ensure the user is notified"
response_criteria_10 = "• Check calendar for 90-minute meeting availability for Monday or Wednesday with check_calendar_availability tool call\n• Send email acknowledging the request and providing availability with write_email tool call"
response_criteria_11 = "• No response needed\n• Ensure the user is notified"
response_criteria_12 = "• No response needed\n• Ensure the user is notified"
response_criteria_13 = "• Acknowledge annual checkup reminder\n• Send email with write_email tool call to acknowledge annual checkup reminder"
response_criteria_14 = "• No response needed\n• Ensure this is ignored"
response_criteria_15 = "• Check calendar for 60-minute meeting availability for Tuesday or Thursday with check_calendar_availability tool call\n• Send calendar invite with schedule_meeting tool call\n• Send email agreeing to collaborate on the joint presentation and notifying that a meeting has been scheduled with write_email tool call"
response_criteria_16 = "• No response needed\n• Ensure the user is notified"
...
def load_triage_datasets():
    ...
    if not client.has_dataset(dataset_name=dataset_name):
        ...
        client.create_examples(
            ...
            outputs=[{"classification": c} for c in triage_outputs_list],
...
def load_response_datasets():
    ...
    if not client.has_dataset(dataset_name=dataset_name):
        ...
        client.create_examples(
            inputs=[{"email_input": email} for email in email_inputs],
            outputs=[{"response_criteria": r} for r in response_criteria_list],
            dataset_id=dataset.id,
        )
    return dataset_name
...
def load_trajectory_datasets():
    ...
    if not client.has_dataset(dataset_name=dataset_name):
        ...
        client.create_examples(
            inputs=[{"email_input": email} for email in email_inputs],
            outputs=[{"trajectory": t} for t in expected_tool_calls],
            dataset_id=dataset.id,
        )
    return dataset_name
